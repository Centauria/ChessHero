...
class Brain:
	...
	def _build_net(self):
		...
		self.stateInput = tf.placeholder(tf.int8, [None, 8, 8], name='input_board')
		self.turnInput = tf.placeholder(tf.int8, [None], name='input_turn')
		self.actionInput = tf.placeholder(tf.float32, [None, 64, 64], name='Q-target')
		self.yInput = tf.placeholder(tf.float32, [None, 8, 8, 2], name='y_input')
		...
		with tf.variable_scope('eval-net'):
			...
			with tf.variable_scope('conv1'):
				w_conv1 = tf.get_variable('w_conv1', [4, 8, 128], initializer=w_initializer, collections=c_names)
				b_conv1 = tf.get_variable('b_conv1', [128], initializer=b_initializer, collections=c_names)
			
			# second layer.
			with tf.variable_scope('conv2'):
				w_conv2 = tf.get_variable('w_conv2', [2, 128, 256], initializer=w_initializer, collections=c_names)
				b_conv2 = tf.get_variable('b_conv2', [256], initializer=b_initializer, collections=c_names)
			
			# first full link layer.
			with tf.variable_scope('fc1'):
				w_fc1 = tf.get_variable('w_fc1', [512, 2000], initializer=w_initializer, collections=c_names)
				b_fc1 = tf.get_variable('b_fc1', [2000], initializer=b_initializer, collections=c_names)
			
			# Q value layer.
			with tf.variable_scope('fc2'):
				w_fc2 = tf.get_variable('w_fc2', [2000, self.actions], initializer=w_initializer, collections=c_names)
				b_fc2 = tf.get_variable('b_fc2', [self.actions], initializer=b_initializer, collections=c_names)
			
			with tf.variable_scope('eval-net-data'):
				h_conv1 = tf.nn.relu(Brain.conv2d(self.stateInput, w_conv1, 2) + b_conv1)
				h_conv2 = tf.nn.relu(Brain.conv2d(h_conv1, w_conv2, 2) + b_conv2)
				h_conv2_flat = tf.reshape(h_conv2, [-1, 512])
		...
		self.stateInputT = tf.placeholder(tf.int8, [None, 8, 8], name='input_board_')
		with tf.variable_scope('target-net'):
			...
			with tf.variable_scope('target-net-data'):
				h_conv1 = tf.nn.relu(Brain.conv2d(self.stateInputT, w_conv1, 2) + b_conv1)
				h_conv2 = tf.nn.relu(Brain.conv2d(h_conv1, w_conv2, 2) + b_conv2)
				h_conv2_flat = tf.reshape(h_conv2, [-1, 512])
				h_fc1 = tf.nn.relu(tf.matmul(h_conv2_flat, w_fc1) + b_fc1)
	...
	def setPerception(self, nextObservation, action_index, reward, terminal, episode):
		...
		if self.e_greedy:
			...
			if self.observe_time < OBSERVE:
				...
				# print info
				if self.print_log:
					print("TIMESTEP", self.timeStep, "/ STATE", state, "/ EPSILON", self.epsilon, "/ EPISODE", episode,
